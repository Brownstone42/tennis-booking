...
<style scoped>
.slot-item { height: 100%; border-radius: 6px; padding: 6px; cursor: pointer; display: flex; flex-direction: column; justify-content: space-between; transition: all 0.2s; position: relative; overflow: hidden; border: 2px solid transparent; }
.slot-item:hover { transform: scale(1.02); z-index: 1; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }

/* Selection Styling */
.slot-item.selected { border-color: #1890ff; background-color: #e6f7ff !important; box-shadow: 0 0 0 2px rgba(24, 144, 255, 0.2); }
.selection-indicator { position: absolute; top: 2px; right: 2px; background: #1890ff; color: white; width: 16px; height: 16px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: bold; z-index: 2; }
